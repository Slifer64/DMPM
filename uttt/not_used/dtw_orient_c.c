...
void anglesOfAxis_to_rotMat(double *angles, double **rotMat)
{
	double a=angles[0], b=angles[1], u=angles[2];
	double kx = cos(a)*abs(sin(b));
	double ky = sin(a)*abs(sin(b));
	double kz = cos(b);
...
double dtw_orient_c(double *s, double *t, int w, int ns, int nt, int k)
{
    double d=0;
        ...
        int nrhs, const mxArray *prhs[])
{
    ...
    dp[0]=dtw_orient_c(s,t,w,ns,nt,k);
