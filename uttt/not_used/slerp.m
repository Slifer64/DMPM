...
%       where theta is the angle between the two unit quaternions,
%       and t is between [0,1]

%       two border cases will be dealt:
...
elseif t==1 % saving calculation time -> where qm=qn
    ...
    C=dot(qi,qn);                  % Calculating the angle between the unit quaternions by dot product

    theta=acos(C);

        if (1 - C) <= eps % if angle theta is close by epsilon to 0 degrees -> calculate by linear interpolation
            qm=qi*(1-t)+qn*t; % avoiding divisions by number close to 0

        elseif (1 + C) <= eps % when theta is close by epsilon to 180 degrees the result is undefined -> no shortest direction to rotate
