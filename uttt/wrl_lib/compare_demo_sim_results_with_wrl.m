function compare_demo_sim_results_with_wrl(wrl_label,T,Pos,Axang, T_sim,Pos_sim,Axang_sim)

    %downsample sim data
    d_f = ceil(length(T_sim)/1000);
    T_sim = T_sim(1:d_f:end);
    Pos_sim = Pos_sim(:,1:d_f:end);
    Axang_sim = Axang_sim(:,1:d_f:end);
